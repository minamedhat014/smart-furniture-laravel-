...
'updated_by' => $this->user,
  ...
  'phone2' ,'city' ,'address']);
     DB::commit();
     $this->emit('gettingCustomerID', $customer->id);
      }catch (\Exception $e) {
        DB::rollBack();
          session()->flash('error', $e); 
     };     
 ...
 public function delete(){
    ...
    if($this->customer_id){
     $customer = Customer::with('stores', 'phone', 'address')
    ->where('id',$this->customer_id)->first();
      ...
      elseif (userFactory()) {
          ...
          ->orWhereHas('phone', function ($subQuery) {
           $subQuery->where('number', 'like', '%' . $this->search . '%');
                  })
          ...
          if($customer === null){
            $customer = Customer::with('stores', 'phone', 'address')->first();
          }
    }
    elseif(CustomerPhone::where('number',$this->search)->exists() or Customer::where('national_id',$this->search)->exists()){
     ...
     ->first();
      ...
      $types=CustomerType::all('name');
        return view('livewire.admin.customers.customer-table',compact('types','customer'));
    }
}
