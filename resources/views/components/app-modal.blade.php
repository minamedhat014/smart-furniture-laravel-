...
<div  wire:ignore.self class="modal fade" id="{{$id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog  @if(Str::startsWith($type, 'store') ||Str::startsWith($type, 'assign') ||Str::startsWith($type, 'update') ||Str::startsWith($type, 'add')) modal-xl  @else modal-dialog-centered @endif">
      <div class="modal-content ">
        ...
        <div class="modal-body">
          <form wire:submit.prevent="{{$type}}" autocomplete="off"  enctype="multipart/form-data">
            @csrf
...
$btnName = implode(' ', $capitalizedPieces);
        @endphp
        <button type="submit" class="btn btn-outline-success  @if(Str::startsWith($type, 'delete')) btn-outline-danger @endif"   @if($type !== 'store' && $type !=='update') data-bs-dismiss="modal" @endif > {{$btnName}}</button>
        </div>
        </form>
      </div>
    </div>
